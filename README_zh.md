...
2. LOCAL 规则针对日志、网络/端口、磁盘空间、Spark/YARN、HDFS、Git 工作区、Kubernetes、Docker、systemd 和目录定位生成最多三条只读建议；网络提示会按 shell 使用 POSIX 的 `ss`/`ip`、`cmd.exe` 的 `netstat`/`ipconfig` 或 PowerShell 的 `Get-NetTCPConnection`/`Get-NetIPConfiguration`，磁盘提示则使用各 shell 对应的本地文件系统和目录盘点命令；
