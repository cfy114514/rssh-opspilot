...
2. A deterministic LOCAL predictor proposes up to three read-only commands for
   common log, network/port, disk-space, memory/process, Spark/YARN, HDFS, Git
   workspace, Kubernetes, Docker, systemd, and directory-orientation situations. Network
   ...
   hints use the corresponding local filesystem, directory, and process inventory
   commands for each shell.
